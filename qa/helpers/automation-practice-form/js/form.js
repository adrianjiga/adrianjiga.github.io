(function () {
  ...
  const DatePicker = {
    init() {
      ...
      document.getElementById('dp-month').addEventListener('change', () => this._renderDays());
      document.getElementById('dp-year').addEventListener('change',  () => this._renderDays());

      document.getElementById('dateOfBirthInput').addEventListener('click', e => {
        e.stopPropagation();
        const popup = document.getElementById('datepicker-popup');
        if (popup.style.display === 'none') { this._renderDays(); popup.style.display = 'block'; }
        else popup.style.display = 'none';
      });

      // Stop clicks inside the popup from bubbling to the document close handler.
      document.getElementById('datepicker-popup').addEventListener('click', e => e.stopPropagation());
    },
    ...
    _renderDays() {
      ...
      for (let b = 0; b < firstWeekday; b++) grid.appendChild(document.createElement('div'));
    ...
    _pickDay(cell) {
      ...
      FormState.selectedDOB = { day: padded, monthName: MONTH_NAMES[month], year };
      document.getElementById('dateOfBirthInput').value = `${padded} ${MONTH_NAMES[month]} ${year}`;
      this.close();
    },
  };
  ...
  const SubjectsInput = {
    init() {
      document.getElementById('subjectsInput').addEventListener('keydown', e => {
        ...
        if (val) { FormState.subjects.push(val); e.target.value = ''; this._render(); }
      });
    },
  ...
  const LocationSelector = {
    init() {
      ...
      document.querySelectorAll('[data-state-idx]').forEach(el => {
        el.addEventListener('click', e => {
          e.stopPropagation();
          this._selectCountry(parseInt(el.dataset.stateIdx, 10), el.textContent.trim());
        });
      });

      document.getElementById('city').addEventListener('click', e => {
        e.stopPropagation();
        if (!FormState.selectedCountry) return;
    ...
    _selectCountry(idx, name) {
      ...
      (COUNTRY_CITIES[name] || []).forEach((city, i) => {
        ...
        opt.setAttribute('data-cy', `city-option-${city.toLowerCase().replace(/\s+/g, '-')}`);
  ...
  const Validator = {
    validate() {
      ...
      ['firstName', 'lastName', 'userNumber'].forEach(id => {
        ...
        if (!el.value.trim()) { this._markError(el); valid = false; }
        else this._clearError(el);
      });

      const checked = document.querySelector('input[name="gender"]:checked');
      for (let i = 1; i <= 3; i++) {
        const lbl = document.querySelector(`label[for="gender-radio-${i}"]`);
        if (!checked) { this._markError(lbl); valid = false; }
  ...
  const SuccessModal = {
    show(rows) {
      document.getElementById('result-tbody').innerHTML = rows
        .map((r, i) => `<tr data-cy="result-row-${i}"><td>${esc(r[0])}</td><td>${esc(r[1])}</td></tr>`)
        .join('');
  ...
  const App = {
    init() {
      ...
      document.getElementById('uploadPicture').addEventListener('change', e => {
      ...
      document.querySelector('[data-cy="close-modal-btn"]')
    ...
    _submit() {
      ...
      const hobbies = [];
      if (document.getElementById('hobbies-checkbox-1').checked) hobbies.push('Sports');
      if (document.getElementById('hobbies-checkbox-2').checked) hobbies.push('Reading');
      if (document.getElementById('hobbies-checkbox-3').checked) hobbies.push('Music');
      ...
      const dob       = FormState.selectedDOB.day
        ...
        : '';
      const stateCity = FormState.selectedCountry && FormState.selectedCity
